...
def describe_image(api_keys, base64_data, mime_type, model, context="", detail="standard", redact=None, verbose=True):
    ...
    if redact:
        redact_types = {
            ...
            "keys": (
                "\n\n⚠️ REDACTION (KEYS): Replace actual secrets and credentials with redaction markers. "
                "This includes: API keys (sk-ant-*, sk-*, AKIA*, etc.) → [REDACTED-KEY], passwords → "
                "[REDACTED-PASS], bearer/auth tokens → [REDACTED-TOKEN], connection strings with "
                "credentials → [REDACTED-CONN], private keys (BEGIN.*KEY) → [REDACTED-PRIVKEY], "
                "webhook URLs containing auth parameters → [REDACTED-WEBHOOK]. "
                "DO NOT redact: hex color codes (#c9a84c), git commit SHAs, UUIDs/session IDs, "
                "SSH fingerprints, port numbers, hostnames, status codes, PIN/parcel numbers, "
                "config names, or any non-secret technical identifiers. When in doubt about whether "
                "a hex string is a secret or a color/SHA, consider its length and context — API keys "
                "are typically 30+ chars, colors are 6 chars, SHAs are 40 chars."
            ),
            "all": (
                "\n\n⚠️ REDACTION (ALL): Replace sensitive information with redaction markers. "
                ...
                "[REDACTED-FINANCIAL], file paths containing usernames → [REDACTED-PATH]. "
                "DO NOT redact: hex color codes (#c9a84c), git commit SHAs, UUIDs/session IDs, "
                "SSH fingerprints, port numbers, hostnames without auth info, status codes, "
                "PIN/parcel numbers, config names, CSS values, version numbers, or any non-secret "
                "technical identifiers. Use length and context to distinguish — API keys are 30+ chars, "
                "colors are 3-6 hex chars, SHAs are 40 chars, UUIDs have dashes in 8-4-4-4-12 pattern. "
                "Keep generic labels, statuses, UI element names, and structural descriptions readable."
            ),
        }
